  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <View style={styles.botIcon}><Ionicons name="sparkles" size={18} color="#4CAF2F" /></View>
        ...
        {messages.map((item, index) => (
          <View key={`${item.type}-${index}`} style={[styles.messageRow, item.type === "user" && styles.messageRowUser]}>
            <View style={[styles.bubble, item.type === "user" ? styles.userBubble : { backgroundColor: colors.aiBubble }]}>
              <Text style={[styles.bubbleText, { color: item.type === "user" ? "#ffffff" : colors.text }]}>{item.text}</Text>
            </View>
          </View>
        ))}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  resultDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: "#4CAF2F" }, resultBadgeText: { color: "#347a22", fontSize: 9, fontWeight: "700" },
  ...
  bubble: { maxWidth: "86%", borderRadius: 13, paddingHorizontal: 13, paddingVertical: 10 }, userBubble: { backgroundColor: "#4CAF2F" }, bubbleText: { fontSize: 12, lineHeight: 18 },
  prompts: { flexDirection: "row", flexWrap: "wrap", gap: 6, paddingHorizontal: 14, paddingBottom: 10 },
  prompt: { borderWidth: 1, borderRadius: 16, paddingHorizontal: 10, paddingVertical: 7 }, promptText: { color: "#4CAF2F", fontSize: 10, fontWeight: "700" },
  ...
  input: { flex: 1, minHeight: 32, fontSize: 12 }, sendButton: { width: 32, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center", backgroundColor: "#4CAF2F" },
